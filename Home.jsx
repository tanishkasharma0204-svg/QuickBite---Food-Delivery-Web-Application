  ...
  return (
    <div className='bg-slate-300 w-full min-h-screen'>
     ...
     {!input?<div className='flex flex-wrap justify-center items-center gap-5 w-[100%]'>
        {Categories.map((item)=>{
            return <div className='w-[140px] h-[150px] bg-white flex flex-col items-start gap-5 p-5 justify-start text-[20px] font-semibold text-gray-700 rounded-lg shadow-lg hover:bg-green-200 cursor-pointer transition-all duration-200' onClick={()=>filter(item.name)}>
            {item.icon}    
            {item.name}
...
<RxCross2 className='w-[30px] h-[30px] text-green-500 text-[18px] font-semibold cursor-pointer hover:text-gray-700' onClick={()=>setShowCart(false)}/>
      </header>
...
<button className="w-[100%] p-3 bg-green-300 rounded-lg hover:bg-green-200 transition-all" onClick={()=>{
